...
createServer(async(req,res)=>{
  ...
  if(req.url?.startsWith('/api/study')){
    if(req.method==='GET'&&req.url.includes('dataset=active')){res.setHeader('Content-Type','application/json');res.end(await readFile(join(import.meta.dirname,'public/data-sets/v3.json')));return}
